...
export class GrovePi implements Sensor {
	...
	public measureBarometricPressure(
		altitude: number
	): Promise<BarometricPressure> {
		return new Promise((resolve, reject) => {
			...
			i2cBus.writeByteSync(
				GrovePi.BMP280_ADDRESS,
				GrovePi.BMP280_CONTROL_REGISTER,
				0x27
			);
			i2cBus.writeByteSync(
				GrovePi.BMP280_ADDRESS,
				GrovePi.BMP280_CONFIGURATION_REGISTER,
				0xa0
			);
			...
			const adcT =
				(readBuffer[3] << 12) + (readBuffer[4] << 4) + (readBuffer[5] >> 4);
