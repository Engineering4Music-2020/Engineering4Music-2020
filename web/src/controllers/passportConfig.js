...
exports.initialize = function (passport) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        passport.use(new LocalStrategy({
            usernameField: "email",
            passwordField: "password",
        }, function (email, password, done) {
            pool_1.pool.query("SELECT * FROM login WHERE email LIKE '" + email + "';", function (err, result) {
                if (err) {
                    throw err;
                }
                if (result.rowCount > 0) {
                    var user_1 = result.rows[0];
                    global.id = user_1.id;
        ...
        passport.deserializeUser(function (id, done) {
            pool_1.pool.query("SELECT * FROM login WHERE id = " + id + ";", function (err, result) {
                if (err) {
                    throw err;
                }
...
exports.checkAuthenticated = function (req, res, next) {
    if (req.isAuthenticated()) {
        ...
        return res.redirect("data" + { user: req.user } + { id: id });
    }
    next();
};
